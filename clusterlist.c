...
int	init_cluster_list(CLUSTER_LIST* clusterList)
{
	if (clusterList == NULL)
		return FAT_ERROR;

	ZeroMemory(clusterList, sizeof(CLUSTER_LIST));
...
int	push_cluster(CLUSTER_LIST* clusterList, SECTOR cluster)
{
	...
	//첫 푸시 또는 아이템이 가득 찼을 때
	if (clusterList->first == NULL ||					
		clusterList->pushOffset == CLUSTERS_PER_ELEMENT)
	{
