...
DWORD get_sector_per_cluster16(UINT64 diskSize, UINT32 bytesPerSector)
{
	DWORD	diskTableFAT16[][2] =
	{
		...
		{ 524288,		8	},
		{ 1048576,		16	},
		//FAT16을 강제 적용하지 않는 한 이 지점 이후는 사용되지 않음
		{ 2097152,		32	},
		{ 4194304,		64	},
		{ 0xFFFFFFFF,	0	}
	};
...
//FAT_BPB에 FATSize16 랑 FATSize32 필드 채우기
...
int fill_bpb(FAT_BPB* bpb, BYTE FATType, SECTOR numberOfSectors, UINT32 bytesPerSector)
{
	...
	if (FATType == FAT32)
	{
		//FAT 활성화. 이건 오직 하나의 FAT만 활성화한다. (중요)
		bpb->BPB32.extFlags = 0x0081;
...
//정의된 파일 시스템의 디스크를 포맷한다
int fat_format(DISK_OPERATIONS* disk, BYTE FATType)
{
	FAT_BPB bpb;

	if (fill_bpb(&bpb, FATType, disk->numberOfSectors, disk->bytesPerSector) != FAT_SUCCESS)
		return FAT_ERROR;
...
//로지컬 클러스터랑 섹션 넘버를 물리적 섹션 넘버로 변환
...
//FAT에서 프리 클러스터들을 찾고 프리 클러스터 리스트에 추가한다
...
int fat_rmdir(FAT_NODE* dir)
{
	...
	//디렉토리인가 확인
	if (!(dir->entry.attribute & ATTR_DIRECTORY))		
		return FAT_ERROR;
...
{
	//디렉토리인지 확인
	if (file->entry.attribute & ATTR_DIRECTORY)
		return FAT_ERROR;
